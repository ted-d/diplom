...
class Bot:
    ...
    def input_looking_age(self, user_id, age):
        ...
        try:
            ...
            self.send_msg(user_id, f' ValueError! VВведен не правильный числовой формат! ')
            return
    ...
    def get_years_of_person(self, bdate: str) -> object:
        ...
        try:
            ...
            key=bdate_splited[1]
            bd = {"1": "января", "2": "февраля", "3": "марта", "4": "апреля", "5": "мая", "6": "июня", "7": "июля",
                  "8": "августа", "9": "сентября", "10": "октября", "11": "ноября", "12": "декабря"}
            return f'День рождения {int(bdate_splited[0])} {bd[key]}.'
    ...
    def get_city(self, user_id):
        ...
        self.send_msg(user_id,
                      ...
                      f'  Введите название города, например: Москва'
                      )
        for event in self.longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                try:
                    ...
                    self.send_msg(user_id,
                                  f'неправильно набран город')


    def looking_gender(self, user_id):
    ...
    def looking_for_persons(self, user_id):
        ...
        res = self.vk_user_got_api.users.search(  # group_token is unavailable for this method users.search.
            ...
            city=city_id,
            hometown=city_title,
            sex=self.looking_gender(user_id),  # 1— женщина, 2 — мужчина, 0 — любой (по умолчанию).
    ...
    def found_person_info(self, show_person_id):
        ...
        #<почему то редко заполняются интересыб поэтому выводить бд не буду_>
        interes = "Информация об интересах не указана."
        if "interests" in res[0]:
            if len(res[0]['interests'])>1:
                interes = res[0]['interests']

        print("Любимые фильмы пользователя:")
        mov="Информация о любимых фильмах не указана."
        if 'movies' in res[0]:
            if len(res[0]['movies'])>1:
                mov = res[0]['movies']




        # Вывод информации о любимой музыке пользователя
        mus = "Информация о любимой музыке не указана."
        if 'music' in res[0]:
            if len(res[0]['music'])>1 :
                mus = res[0]['music']
        ...
        try:
            ...
            pass
        print(f'{first_name} {last_name}, {age}, {city}. {vk_link},{mus},{mov},{interes}')
        return f'{first_name} {last_name}, {age}, {city}. {vk_link}\nЛюбимая музыка: {mus},\nЛюбимое кино: {mov},\n ' \
               f'Мои интересы: {interes}'
